...
export function EntityPage({ config, useStore }: { config: EntityConfig; useStore: EntityStore }) {
  ...
  return <main className="workspace">
    ...
    <section className="table-shell" aria-busy={loading}><table><thead><tr><th>编码</th><th>名称</th><th>状态</th><th>风险</th><th>责任人</th><th>指标</th>{config.key === 'faultEvent' && <th>累计次数</th>}{config.key === 'faultEvent' && <th>最近上报</th>}<th>更新时间</th><th>操作</th></tr></thead><tbody>
      {items.map((item) => { const next = nextStatus(item.status, config.statuses); return <tr key={item.id}><td><strong>{item.code}</strong></td><td>{item.name}<small>{item.facility}</small></td><td><StatusBadge status={item.status}/></td><td>{item.riskLevel}</td><td>{item.owner}</td><td>{item.metricValue} {item.metricUnit}</td>{config.key === 'faultEvent' && <td>{item.occurrenceCount}</td>}{config.key === 'faultEvent' && <td>{formatDate(item.lastReportedAt)}</td>}<td>{formatDate(item.updatedAt)}</td><td>{next ? <button className="table-action" disabled={!canWrite} onClick={() => setPending({ item, status: next })}>{canWrite ? '推进至' : '无权限推进至'} {next}</button> : <span className="muted">流程结束</span>}</td></tr>; })}
      {!items.length && !loading && <tr><td colSpan={config.key === 'faultEvent' ? 10 : 8} className="empty">暂无记录</td></tr>}
