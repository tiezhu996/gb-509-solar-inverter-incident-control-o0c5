...
type InverterUnitHandler struct{ service service.InverterUnitService }

func NewInverterUnitHandler(s service.InverterUnitService) *InverterUnitHandler {
	return &InverterUnitHandler{service: s}
}

func (h *InverterUnitHandler) Register(group *gin.RouterGroup) {
	resource := group.Group("/inverters")
	resource.GET("", h.list)
...
func (h *InverterUnitHandler) list(c *gin.Context) {
	query := bindPage(c)
...
func (h *InverterUnitHandler) get(c *gin.Context) {
	id, ok := parseID(c)
	if !ok {
		return
	}
...
func (h *InverterUnitHandler) create(c *gin.Context) {
	var input dto.CreateInverterUnit
...
func (h *InverterUnitHandler) update(c *gin.Context) {
	id, ok := parseID(c)
	if !ok {
		return
	}
	var input dto.UpdateInverterUnit
	if err := c.ShouldBindJSON(&input); err != nil {
		util.Fail(c, http.StatusBadRequest, "invalid_request", err.Error())
		return
	}
...
func (h *InverterUnitHandler) transition(c *gin.Context) {
	id, ok := parseID(c)
	if !ok {
		return
	}
...
func (h *InverterUnitHandler) remove(c *gin.Context) {
