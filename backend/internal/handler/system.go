...
type SystemHandler struct {
	...
	solarSite        service.SolarSiteService
	inverterUnit     service.InverterUnitService
	faultEvent       service.FaultEventService
	mitigationAction service.MitigationActionService
	db               *gorm.DB
	redis            *redis.Client
}

func NewSystemHandler(security service.SecurityService, solarSite service.SolarSiteService, inverterUnit service.InverterUnitService, faultEvent service.FaultEventService, mitigationAction service.MitigationActionService, db *gorm.DB, redisClient *redis.Client) *SystemHandler {
	return &SystemHandler{security: security, solarSite: solarSite, inverterUnit: inverterUnit, faultEvent: faultEvent, mitigationAction: mitigationAction, db: db, redis: redisClient}
}
...
func (h *SystemHandler) Overview(c *gin.Context) {
	...
	solarSiteCounts, err := h.solarSite.StatusCounts(ctx)
	if err != nil {
		handleError(c, err)
		return
	}
	result["sites"] = solarSiteCounts

	inverterUnitCounts, err := h.inverterUnit.StatusCounts(ctx)
	if err != nil {
		handleError(c, err)
		return
	}
	result["inverters"] = inverterUnitCounts

	faultEventCounts, err := h.faultEvent.StatusCounts(ctx)
	if err != nil {
		handleError(c, err)
		return
	}
	result["faults"] = faultEventCounts

	mitigationActionCounts, err := h.mitigationAction.StatusCounts(ctx)
	if err != nil {
		handleError(c, err)
		return
	}
	result["actions"] = mitigationActionCounts
