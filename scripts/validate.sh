...
i=0
until curl -fsS "http://127.0.0.1:${BACKEND_PORT:-19509}/healthz" >/dev/null; do
  i=$((i+1)); [ "$i" -lt 60 ] || { docker compose logs; exit 1; }; sleep 2
done
curl -fsS "http://127.0.0.1:${FRONTEND_PORT:-18509}/" >/dev/null
token=$(curl -fsS -X POST "http://127.0.0.1:${BACKEND_PORT:-19509}/api/auth/login" -H 'Content-Type: application/json' -d '{"username":"admin","password":"Admin123!"}' | sed -n 's/.*"token":"\([^"]*\)".*/\1/p')
[ -n "$token" ]
viewer_token=$(curl -fsS -X POST "http://127.0.0.1:${BACKEND_PORT:-19509}/api/auth/login" -H 'Content-Type: application/json' -d '{"username":"viewer","password":"Admin123!"}' | jq -er '.data.token')
curl -fsS "http://127.0.0.1:${BACKEND_PORT:-19509}/api/overview" -H "Authorization: Bearer $token" >/dev/null
...
viewer_write_status=$(curl -sS -o /dev/null -w '%{http_code}' -X POST "http://127.0.0.1:${BACKEND_PORT}/api/$resource" -H "Authorization: Bearer $viewer_token" -H 'Content-Type: application/json' -d "$payload")
...
action_code="ACTION-SMOKE-$(date +%s)-$$"
action_payload=$(printf '{"code":"%s","name":"Remote action validation","description":"Validates two-step remote confirmation","facility":"Validation Site","owner":"admin","category":"remote","riskLevel":"high","metricValue":1,"metricUnit":"unit","effectiveAt":"%s","evidence":"scripts/validate.sh","relatedCode":"FAULT-SMOKE"}' "$action_code" "$now")
action_created=$(curl -fsS -X POST "http://127.0.0.1:${BACKEND_PORT}/api/actions" -H "Authorization: Bearer $token" -H 'Content-Type: application/json' -d "$action_payload")
action_id=$(printf '%s' "$action_created" | jq -er '.data.id')
action_version=$(printf '%s' "$action_created" | jq -er '.data.version')
unconfirmed_action=$(printf '{"status":"confirmed","expectedVersion":%s,"reason":"first confirmation only"}' "$action_version")
unconfirmed_status=$(curl -sS -o /dev/null -w '%{http_code}' -X POST "http://127.0.0.1:${BACKEND_PORT}/api/actions/$action_id/transition" -H "Authorization: Bearer $token" -H 'Content-Type: application/json' -d "$unconfirmed_action")
[ "$unconfirmed_status" = "422" ]
confirmed_action=$(printf '{"status":"confirmed","expectedVersion":%s,"reason":"independent second confirmation","confirmed":true}' "$action_version")
curl -fsS -X POST "http://127.0.0.1:${BACKEND_PORT}/api/actions/$action_id/transition" -H "Authorization: Bearer $token" -H 'Content-Type: application/json' -d "$confirmed_action" | jq -e '.data.status == "confirmed"' >/dev/null
